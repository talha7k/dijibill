...
func (d *Database) GetSystemSettings() (*SystemSettings, error) {
	query := `SELECT id, currency, language, timezone, date_format, invoice_language, zatca_enabled, auto_backup, backup_frequency, last_backup_time, created_at, updated_at FROM system_settings LIMIT 1`

	var s SystemSettings
	err := d.db.QueryRow(query).Scan(&s.ID, &s.Currency, &s.Language, &s.Timezone, &s.DateFormat, &s.InvoiceLanguage, &s.ZatcaEnabled, &s.AutoBackup, &s.BackupFrequency, &s.LastBackupTime, &s.CreatedAt, &s.UpdatedAt)
	if err != nil {
		return nil, err
	}
	return &s, nil
